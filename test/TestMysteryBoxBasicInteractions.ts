...
describe("MysteryBoxBasicInteractions", () => {
  ...
  it("Should not be able to call initialize", async () => {
    await expect(mbContract.connect(contractCreator).initialize()).to.be.rejectedWith(
      "Initializable: contract is already initialized",
    );
    await expect(mbContract.connect(user_1).initialize()).to.be.rejectedWith(
      "Initializable: contract is already initialized",
    );
    await expect(mbContract.connect(user_2).initialize()).to.be.rejectedWith(
      "Initializable: contract is already initialized",
    );
  });
  ...
  it("Should createBox reject invalid parameters", async () => {
    {
      ...
      ).to.be.rejectedWith("invalid end time");
    }
    {
      const invalidParameter = { ...createBoxPara, payment: [] };
      await expect(
        mbContract.connect(user_1).createBox.apply(null, Object.values(invalidParameter)),
      ).to.be.rejectedWith("invalid payment");
    }
    {
      ...
      await expect(
        mbContract.connect(user_1).createBox.apply(null, Object.values(invalidParameter)),
      ).to.be.rejectedWith("not enumerable nft");
    }
    {
      const invalidParameter = { ...createBoxPara, sell_all: false, nft_id_list: [] };
      await expect(
        mbContract.connect(user_1).createBox.apply(null, Object.values(invalidParameter)),
      ).to.be.rejectedWith("empty nft list");
    }
    {
      ...
      ).to.be.rejectedWith("not ApprovedForAll");
    }
    {
      const invalidParameter = { ...createBoxPara, sell_all: true };
      ...
      ).to.be.rejectedWith("no nft owned");
    }
    {
  ...
  it("Should openBox reject invalid parameters", async () => {
    {
      ...
      await expect(mbContract.connect(user_3).openBox.apply(null, Object.values(invalidParameter))).to.be.rejectedWith(
